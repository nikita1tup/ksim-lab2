...
rabbits-own [ energy health gender-m ]
...
to reproduce     ;; rabbit procedure
  if energy > birth-threshold and health = max-health
  [
    if gender-m = true and any? rabbits in-radius 1 with [gender-m = false and health = max-health and energy > birth-threshold]
    [
      ;; male rabbits only lose energy
      set energy energy / 2
    ]
    if gender-m = false and any? rabbits in-radius 1 with [gender-m = true and health = max-health and energy > birth-threshold]
    [
